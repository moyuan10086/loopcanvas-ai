import { ArrowLeft, ArrowLeftRight, ArrowRight, BookOpen, CheckSquare, ClipboardPaste, Download, FolderPlus, History, LoaderCircle, Music2, Plus, SlidersHorizontal, Sparkles, Trash2, Upload, VideoIcon } from "lucide-react";
...
import { normalizeVideoFrameMode, videoFrameModeError, videoFrameModeLabel, videoFrameReferenceLabel } from "@/lib/video-frame-mode";
...
type GenerationLogConfig = Pick<AiConfig, "model" | "videoModel" | "size" | "vquality" | "videoSeconds" | "videoFrameMode" | "videoGenerateAudio" | "videoWatermark">;
...
export default function VideoPage() {
    ...
    const swapFirstLastFrames = () => {
        setReferences((value) => (value.length < 2 ? value : [value[1], value[0], ...value.slice(2)]));
    };
    ...
    const buildRequestSnapshot = () => {
        ...
        const frameModeError = videoFrameModeError(effectiveConfig.videoFrameMode, references.length);
        if (frameModeError) {
            message.error(frameModeError);
            return null;
        }
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        if (log.config.videoFrameMode) updateConfig("videoFrameMode", log.config.videoFrameMode);
                    ...
                    <div className="thin-scrollbar flex flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="min-w-0">
                                <div className="mb-2 flex items-center justify-between gap-3">
                                    <span className="text-base font-semibold">{videoFrameModeLabel(effectiveConfig.videoFrameMode, isSeedanceVideoConfig(effectiveConfig))}</span>
                                    <div className="flex gap-2">
                                ...
                                <div className="hover-scrollbar hover-scrollbar-hint flex min-h-24 w-full min-w-0 max-w-full gap-2 overflow-x-scroll overflow-y-hidden rounded-lg border border-dashed border-stone-300 p-2 pb-3 overscroll-x-contain dark:border-stone-700">
                                    {references.map((item, index) => {
                                        const firstLastMode = normalizeVideoFrameMode(effectiveConfig.videoFrameMode) === "first-last-frame";
                                        const frameLabel = videoFrameReferenceLabel(effectiveConfig.videoFrameMode, index);
                                        const highlightedFrame = firstLastMode && index < 2;
                                        return (
                                            <Fragment key={item.id}>
                                                {firstLastMode && index === 1 ? (
                                                    <button
                                                        type="button"
                                                ...
                                                <div
                                                    className={`group relative shrink-0 overflow-hidden rounded-md border ${highlightedFrame ? "size-24 border-2 border-stone-500 dark:border-stone-300" : "size-20 border-stone-200 dark:border-stone-800"}`}
                                                >
                                                    <img src={item.dataUrl} alt={item.name} className="size-full object-cover" />
                                                    <span className={`absolute left-1 top-1 rounded px-2 py-1 text-xs font-semibold text-white shadow-sm ${highlightedFrame ? (index === 0 ? "bg-sky-600/95" : "bg-amber-600/95") : "bg-black/60"}`}>
                                                        {frameLabel}
                                                    </span>
                                    ...
                                    {!references.length ? (
                                        <div className="flex min-w-full items-center justify-center text-sm text-stone-500">
                                            {normalizeVideoFrameMode(effectiveConfig.videoFrameMode) === "first-last-frame"
                                                ? "请添加首帧和尾帧图片"
                                                : normalizeVideoFrameMode(effectiveConfig.videoFrameMode) === "first-frame"
                                                  ? "请添加首帧图片"
                                                  : isSeedanceVideoConfig(effectiveConfig)
                                                    ? "全能参考：可添加图片、视频和音频，参考图最多 9 张"
                                                    : "暂无参考图，最多 9 张"}
                                        </div>
                                    ) : null}
                                </div>
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {videoFrameModeLabel(effectiveConfig.videoFrameMode, isSeedanceVideoConfig(effectiveConfig))} · {videoResolutionLabel(effectiveConfig.vquality, model)} ·{" "}
                                    {videoSizeLabel(effectiveConfig.size)} · {videoSecondsLabel(effectiveConfig.videoSeconds, model)}
                                </span>
...
function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        videoFrameMode: normalizeVideoFrameMode(log.config?.videoFrameMode),
