...
- 参考 Linear、Vercel、Coze、Arco、Shadcn 等外部产品时，默认只借鉴布局、间距、边框、阴影、排版与交互状态；除非用户明确要求，不得替换项目现有主题色、主按钮颜色或深浅色配色。
...
- 直接编辑已有图片时必须始终把当前节点图片作为主参考；显式 `@` 或“隐式参考”连接的图片只能追加为辅助参考，不能替换或排除源图。循环把图片节点作为纯步骤模板执行时除外。
- 循环图片编辑流程的固定参考必须由循环节点“固定参考图”选项显式指定，不要根据连线位置猜测；支持同时选择多张固定图，所选图片每轮全部复用且不参与轮换计数，左侧附加图片仍按轮次轮换，提示词中的 `@固定图` 指代整组固定图。
- 连接本地 Canvas Agent 后，画布项目、节点媒体和 API 调用统计应同步保存到 `~/.infinite-canvas/`；浏览器 IndexedDB 只作为离线缓存。改动持久化逻辑时必须避免初始化空状态覆盖后端，并验证图片、视频和音频能够跨浏览器恢复。“我的素材”仍主要保存在浏览器本地，不要误写成已支持云同步。
