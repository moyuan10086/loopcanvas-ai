...
```text
请从 https://github.com/moyuan10086/loopcanvas-ai.git 安装 LoopCanvas AI Codex 插件。
...
```bash
mkdir -p ~/plugins
git clone https://github.com/moyuan10086/loopcanvas-ai.git ~/plugins/loopcanvas-ai
```
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "local",
        "path": "./plugins/loopcanvas-ai/plugins/infinite-canvas"
      },
      "policy": {
...
```text
<画布网页地址>/canvas?mode=recent&agentUrl=<Local URL>&agentToken=<Connect token>
```

`mode=recent` 会打开最近使用的画布并连接本地 Agent；只有没有任何画布时才自动新建。需要创建空白画布时再使用 `mode=new`。
